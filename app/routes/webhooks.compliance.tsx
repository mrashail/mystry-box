...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (topic === "CUSTOMERS_REDACT") {
    ...
    if (customerId) {
      // Both tables key rows by customerId, so a redaction must clear both —
      // MysteryCustomerHistory records which variants the customer received.
      await prisma.$transaction([
        prisma.promotionUsage.deleteMany({ where: { shop, customerId } }),
        prisma.mysteryCustomerHistory.deleteMany({ where: { shop, customerId } }),
      ]);
    }
  }
